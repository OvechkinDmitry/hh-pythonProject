...
class report:
    def __init__(self, vacancies_count_by_years, vacancies_count_by_years_for_profession,
                 salary_by_years,salary_by_years_for_profession, vacancies_count_by_cities,
                 vacancies_share_by_cities, salary_by_cities,profession):
    ...
    def remake_to_percantage(self, ws, column_index):
        for row in range(2, ws.max_row + 1):
            ws.cell(column=column_index, row=row).value = \
                str(round((ws.cell(column=column_index, row=row).value * 100), 2)).replace(".", ",") + "%"
        return ws
...
profession = ['analytic', 'аналитик', 'analyst', 'аналітик']
